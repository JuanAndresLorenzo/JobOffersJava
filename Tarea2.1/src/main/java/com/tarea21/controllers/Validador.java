...
public class Validador {
	...
	public Boolean validarCorreo(String valor) {
		...
		if (valor.matches("^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$")) {
	...
	public Boolean validarClaveFuerte(String valor) {
		...
		if (valor.matches("^(?=.*\\d)(?=.*[\\u0021-\\u002b\\u003c-\\u0040])(?=.*[A-Z])(?=.*[a-z])\\S{8,20}$")) {
		...
		/*
								"La contraseña debe contener obligatoriamente para que sea fuerte: "
										+ "entre 8 y 20 caracteres, " + "al menos un dígito, " + "al menos una minúscula, "
										+ "al menos una mayúscula y " + "al menos un caracter no alfanumérico (_-.*).");*/

				
	}
	
	public Boolean validarTipoImagen(String extension) {
